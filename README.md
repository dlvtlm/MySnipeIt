...
6. **A fallback frame is a *skip frame*, and the lock must be held across it, not resolved against it.** The Pi skips tracking for a frame when it cannot join that frame to a servo pose, or when the frame was captured while the arm was still settling after a large commanded slew. Both cases forward the Orin's raw boxes with `confirmed` omitted. So on a skip frame the locked track's stable id is absent **by construction**, even while that track is alive and coasting on the Pi. `List<DetectedTarget>.isFallbackFrame()` identifies one, `DashboardScreen` holds the last tracked-frame locked target across it, and the lock-loss timer ignores it. Without that, the firing card blinked off on every scan step and on every slew. An empty `detections: []` is deliberately **not** a skip frame, because that is the Pi saying "clear all boxes", which is real information.

**When a lock does die.** The Pi has no `lock_lost` message. Worse, when its tracker drops the locked track the Pi stays locked on the now dead id and the servo freezes at its last commanded pose until the app sends `unlock`, a new `lock`, or `set_servo_angles`. The id disappearing from the stream is the app's only signal, and it looks exactly like a target walking behind a wall. So `SniperViewModel.lockLost` turns the state chip to **LOCK LOST** once the id has been missing from tracked frames for longer than the Pi's own 1.5 s coast budget, and stops there. It does not clear the lock, because a brief occlusion is not a lost target and because pressing UNLOCK is also what unfreezes the rig. It re-acquires nothing: that would be app-side tracking under another name, which is the thing this design removed.

**SLEW cancels a lock.** `set_servo_angles` clears the Pi's lock unconditionally and sends nothing back to say so. The acoustic SLEW button is a `set_servo_angles`, so `acceptAudioAlert()` clears the app's own lock state to match. The button is normally only offered when nothing is locked, so this covers the race where a lock lands between the alert being drawn and the operator tapping it.
