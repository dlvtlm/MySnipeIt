...
- **`RigGeometry` constants reflect the real rig** — `COMPASS_ON_FIXED_BASE = false` (the compass is bolted to the moving camera arm, so its reading IS the camera's pointing direction — servo pan is NOT added on top), servo pan/tilt centered at 90°, declination 0, `MIC_ARRAY_OFFSET_DEG = 0` (compass↔mic-array, used for acoustic-event WORLD bearing display; correct only when mic 0° is mechanically aligned with compass north — design intent, but field-test the assembled rig), and `MIC_TO_SERVO_OFFSET_DEG = 90.0` (mic↔servo, used for the SLEW path; per Pi-side spec, mic 0° aligns to servo 90°). One field test against the real rig may reveal small mounting errors in the first set; the 90° mic-to-servo offset is mechanical design and is unlikely to change. Tests in `TargetLocalizerTest` encode the localizer's assumptions.
