...
- **`RigGeometry` constants are UNVERIFIED** — `TargetLocalizer.kt` assumes compass on the fixed base, servo pan/tilt centered at 90°, declination 0, `MIC_ARRAY_OFFSET_DEG = 0` (compass↔mic-array, used for acoustic-event WORLD bearing display; correct only when mic 0° is mechanically aligned with compass north — design intent, but field-test the assembled rig), and `MIC_TO_SERVO_OFFSET_DEG = 90.0` (mic↔servo, used for the SLEW path; per Pi-side spec, mic 0° aligns to servo 90°). One field test against the real rig must confirm/flip the first set; the 90° mic-to-servo offset comes from the team's mechanical design and is unlikely to change. Tests in `TargetLocalizerTest` encode the localizer's assumptions.
