...
- **Lock/unlock: the card's LOCK button OR double-tap the bbox** — both run the same `onLockClick` (which guards on `confirmed`); the double-tap (min 48dp hit area for distant targets) exists because the card/button can be covered by HUD bars. The bottom sensor strip overlays the video region, so `DashboardScreen` passes its height (`SensorStripHeight`, 68dp) into `TacticalVideoPlayer` as `bottomOverlayObstruction` (plus a `topOverlayObstruction`, 0 today — the TopBar stacks above the video, it doesn't overlay); the player converts these to per-edge intrusions into the centered 16:9 video box (letterbox slack absorbs part of each bar first) and the info card flips above the bbox / clamps into the uncovered zone so its button always stays pressable. The card is also clamped horizontally so edge targets don't wrap their text.
