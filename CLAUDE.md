...
- `lockLost: StateFlow<Boolean>` — DISPLAY state: true when `selectedTargetId` is set but that id has been absent from every **tracked** frame for longer than `lockLostGraceMs` (1.5 s = the Pi's `MAX_COAST_MS`, past which the Pi has deleted the track and a re-detection would arrive under a new id). Fed by `refreshLockPresence()`, which runs on each detection batch **and** on a 500 ms tick (so the flag can trip with no new frame arriving). **Skip frames never count** — they carry the Orin's per-frame ids, so the locked id is absent by construction and counting them would raise the warning on every scan step and slew. The flag never clears the lock: it drives the dashboard's LOCK LOST chip and the operator releases with UNLOCK. It is not tracking — no re-acquire, no re-ID, no id rewriting.
...
3. **Empty `detections: []` clears the overlay immediately** — the Pi signals "clear all boxes" explicitly.
   **Never resolve the lock against a skip frame.** Because a skip frame carries per-frame Orin ids, the locked stable id is *guaranteed* absent from it even while the Pi's track is alive and coasting. `List<DetectedTarget>.isFallbackFrame()` (in `Target.kt`) identifies one — every detection has `confirmed == null`, and an EMPTY batch is deliberately not a skip frame (that's the Pi's explicit clear). `DashboardScreen` holds the last tracked-frame `lockedTarget` across skip frames so the firing card doesn't blink off on every scan step and slew, and `SniperViewModel.refreshLockPresence()` ignores them for the same reason.
...
- **`RigGeometry` constants reflect the real rig** — `COMPASS_ON_FIXED_BASE = false` (the compass is bolted to the moving camera arm, so its reading IS the camera's pointing direction — servo pan is NOT added on top), servo pan/tilt centered at 90°, declination 0, and the mic↔servo mapping used by the SLEW path is `servo = SERVO_HORIZONTAL_CENTER_DEG - mic_azim` (**subtract, not add** — mic azimuth is positive to the RIGHT while the pan servo turns the camera LEFT as its angle rises, so the frames run in opposite senses; a clap at mic +60 drives the servo to 30). This replaced a `mic + 90` that slewed to the mirror image of every contact. The sign is empirical, verified on the rig, and flips back if the Pi changes its pan convention. There's no compass↔mic-array constant — the acoustic-bearing path uses an operator-calibrated `tripodWorldBearingDeg` (set via MENU → Calibrate Bearing) instead of a static constant, because the compass is on the moving head while the mics are on the fixed tripod, so their relationship isn't a mechanical constant.
...
- **`set_servo_angles` silently cancels the Pi's lock.** Confirmed by the Pi dev: on receipt the Pi clears `b->locked` unconditionally and kicks the servo FSM out of `TARGET_LOCK` (`eSERVO_EVENT_NOISE_DETECTED`), and it sends the app NO message about it. The acoustic SLEW is a `set_servo_angles`, so `SniperViewModel.acceptAudioAlert()` calls `deselectTarget()` after sending — otherwise the app would keep claiming TARGET LOCKED over a rig that has gone back to scanning. In practice the SLEW button is only offered while nothing is locked (`AudioAlert.isInteractive`), so this is the race guard, not the normal path.
- **A dead locked track leaves the Pi locked on a dead id, servo frozen.** There is no `lock_lost` message type in the protocol. When the Pi's tracker drops the locked track, `b->locked` stays true, `select_detection_index` returns -1 every frame, and the servo holds its last commanded pose until the app sends `unlock`, a new `lock`, or `set_servo_angles`. The id vanishing from the stream is the app's ONLY signal, and it is indistinguishable from the target simply leaving frame — which is why `lockLost` warns (LOCK LOST chip) instead of auto-clearing: releasing is the operator's call, and UNLOCK is also what unfreezes the rig. The underlying id churn is a Pi-side defect (filed there as S1/S2), not an app bug.
